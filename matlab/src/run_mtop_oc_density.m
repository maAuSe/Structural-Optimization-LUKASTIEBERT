...
if nargin < 2 || isempty(classicalDensityResult)
  classicalDensityResult = load_or_run_classical_density_reference(cfg);
end

p = cfg.parameters;

problem = struct();
problem.id = 'mtop_oc_density';
...
fprintf('\nRunning assignment step 3: MTOP OC density baseline\n');
...
result = solve_mtop_mbb_oc_density(problem);
result.problem = problem;

densityDifference = result.xPhys - classicalDensityResult.xPhys;
result.reference = make_reference_summary(classicalDensityResult);
...
result.fineMeshComplianceRatio = result.fineMeshCompliance / classicalDensityResult.finalCompliance;
result.speedupAgainstClassical = classicalDensityResult.elapsedSeconds / result.elapsedSeconds;
...
export_mtop_oc_figures(cfg.paths.figures, result, classicalDensityResult, densityDifference);

fprintf('\nSaved step 3 (MTOP) result data:\n');
fprintf('  %s\n', resultPath);
fprintf('  %s\n', historyPath);
fprintf('  %s\n', summaryPath);
fprintf('Saved step 3 (MTOP) figures in:\n');
fprintf('  %s\n\n', cfg.paths.figures);

end

function classicalResult = load_or_run_classical_density_reference(cfg)

resultPath = fullfile(cfg.paths.results, 'classical_oc_density.mat');
if isfile(resultPath)
  data = load(resultPath, 'result');
  classicalResult = data.result;
else
  fprintf('Classical density reference result not found. Running step 3 classical run first.\n');
  classicalResult = run_classical_oc_density(cfg);
end

end

function result = solve_mtop_mbb_oc_density(problem)
...
while change > tol && iter < maxIter
  ...
  while (l2 - l1) / (l1 + l2) > 1e-3
    ...
    if sum(xnewPhys(:)) > volfrac * densityNelx * densityNely
      l1 = lmid;
    else
      l2 = lmid;
    end
...
if ~result.converged
  warning('run_mtop_oc_density:notConverged', ...
...
fprintf(fid, 'MTOP OC density baseline\n');
...
caxis([0 1]);
axis equal tight off;
title('Classical density-filtered design');
nexttile;
imagesc(1 - result.xPhys);
colormap(gca, gray);
caxis([0 1]);
axis equal tight off;
title('MTOP density-filtered design');
nexttile;
...
export_figure(fig, figuresDir, [baseId '_design_difference']);
close(fig);
...
ylabel('Compliance');
title('Compliance convergence by iteration (density filtering)');
...
title('Volume convergence by iteration (density filtering)');
legend({'Classical', 'MTOP', 'Target'}, 'Location', 'best');
export_figure(fig, figuresDir, [baseId '_convergence_iter_compare']);
close(fig);
...
ylabel('Compliance');
title('Compliance convergence by time (density filtering)');
legend({'Classical', 'MTOP'}, 'Location', 'northeast');
nexttile;
...
xlabel('Computation time [s]');
ylabel('Volume fraction');
title('Volume convergence by time (density filtering)');
legend({'Classical', 'MTOP', 'Target'}, 'Location', 'best');
export_figure(fig, figuresDir, [baseId '_convergence_time_compare']);
close(fig);
