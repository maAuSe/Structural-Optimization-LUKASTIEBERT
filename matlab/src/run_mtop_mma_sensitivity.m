...
problem = struct();
problem.id = 'mtop_mma_sensitivity';
...
fprintf('\nRunning assignment step 4: MTOP MMA sensitivity\n');
...
fprintf('  Filter radius: %.4f density cells (sensitivity filter)\n\n', problem.rmin);

result = solve_mtop_mma_sensitivity(problem);
...
fprintf('\nSaved step 4 (MTOP MMA sensitivity) result data:\n');
fprintf('  %s\n', resultPath);
fprintf('  %s\n', historyPath);
fprintf('  %s\n', summaryPath);
fprintf('Saved step 4 (MTOP MMA sensitivity) figures in:\n');
fprintf('  %s\n\n', cfg.paths.figures);

end

function result = solve_mtop_mma_sensitivity(problem)
...
if ~result.converged
  warning('run_mtop_mma_sensitivity:notConverged', ...
...
fprintf(fid, 'MTOP MMA sensitivity\n');
