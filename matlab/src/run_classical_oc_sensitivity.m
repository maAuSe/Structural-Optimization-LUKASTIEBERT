...
problem = struct();
problem.id = 'classical_oc_sensitivity';
...
fprintf('\nSaved step 1 result data:\n');
fprintf('  %s\n', resultPath);
fprintf('  %s\n', historyPath);
fprintf('  %s\n', summaryPath);
fprintf('Saved step 1 figures in:\n');
fprintf('  %s\n\n', cfg.paths.figures);
...
fprintf(fid, 'Classical OC sensitivity baseline\n');
...
fprintf(fid, 'Sensitivity filter radius: %.6f elements\n', result.problem.rmin);
...
export_figure(fig, figuresDir, 'classical_oc_sensitivity_design');
close(fig);
...
export_figure(fig, figuresDir, 'classical_oc_sensitivity_convergence_iter');
close(fig);
...
export_figure(fig, figuresDir, 'classical_oc_sensitivity_convergence_time');
close(fig);
