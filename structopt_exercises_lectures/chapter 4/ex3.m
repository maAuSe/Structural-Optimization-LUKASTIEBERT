%EX3
%
%   Exercise 4.6: use of SLP, CONLIN, and MMA to solve a two-segment cantilever
...
% PROBLEM FORMULATION
xmin = 0.1;       % Lower bound for x
xmax = 10;        % Upper bound for x
x = [5;5];        % Initial value
...
sinit = 0.1;      % Asymptotes relative initial position.
...
[X1,X2] = ndgrid(0:0.05:12,0:0.05:12);
F0 = X1+X2;
F = 1./X1.^3+7./X2.^3-1;
figure;
...
xlim([0 12]);
ylim([0 12]);
[~,h1] = contour(X1,X2,F0,[-10:2:24],'LineWidth',1.5,'Color',darkblue);
...
while change>tol
  ...
  [~,hf0] = contour(X1,X2,F0,[-10:2:24],'Color',darkblue);
