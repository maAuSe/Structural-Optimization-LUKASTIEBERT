%EX5B
%
%   Topology optimization considering different load cases - the sum of the
...
while change>tol
  ...
  df0dx = (dc1dx(:)'+dc2dx(:)')/100;
  dfdx = dvdx(:)'/volfrac;

  % MMA UPDATE
  [xnew,~,~,~,mmaparams,~,change,history] = mma(x(:),xmin,xmax,f0,f,df0dx,dfdx,mmaparams,'silent');
