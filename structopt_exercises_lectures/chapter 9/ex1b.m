...
%   sensitivity or density filtering - plot densities and displacements.
...
while change>tol
  ...
  % PLOT DENSITIES AND DISPLACEMENTS
  if iter==1
    defscale = max(nelx,nely)/max(abs(U(:,1)))/2;
  end
  [xx,yy] = meshgrid(1:nelx,nely:-1:1);
  xx = repmat(xx(:),1,4)+repmat([-1 0 0 -1],nelx*nely,1)+defscale*(U(edofMat(:,1:2:end)));
  yy = repmat(yy(:),1,4)+repmat([-1 -1 0 0],nelx*nely,1)+defscale*(U(edofMat(:,2:2:end)));
  newplot;
  colormap(flipud(gray));
  patch(xx',yy',xPhys(:)','EdgeColor',[0.5 0.5 0.5]);
  caxis([0 1]);
  axis('equal');
  axis('off');
  drawnow;
