%EX2D
%
%   Size and shape optimization of a truss girder for minimum compliance subject
...
Vmax = 0.008;                                  % Maximum volume

% PHYSICAL AND ADJOINT LOAD VECTOR
...
while true
  ...
  U = K\F;
  C = F'*U;                       % Compliance
  dCdx = zeros(1,n);              % Compliance sensitivities
  for k = 1:n
    dCdx(k) = -U'*dKdx{k}*U;
  end
  f0 = C;
  ...
  % PLOT CURRENT DESIGN
  if ~isinf(change)
    plotdisp(Nodes,Elements,Types,DOF,U,'DispMax','off','DispScal',50); hold('on');
