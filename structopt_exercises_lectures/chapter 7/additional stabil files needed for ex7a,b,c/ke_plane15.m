...
for iXi=1:Options.nXi
    ...
    if nargout>=3 && (any(dNodedx(:)~=0) || any(dSectiondx(:)~=0))
      ...
      for iVar = 1:nVar
        ...
        dBdx=zeros(3,30);
        dBdx(1,1:2:29)=dGdx(1,:);
        dBdx(2,2:2:30)=dGdx(2,:);
        dBdx(3,1:2:29)=dGdx(2,:);
        dBdx(3,2:2:30)=dGdx(1,:);
        dKedx{iVar} = dKedx{iVar} + (dBdx.'*C*B*detJ + B.'*C*dBdx*detJ + B.'*C*B*ddetJdx)*H(iXi);
      end
    end
