function [Ke,Me,dKedx] = ke_shell6(Node,Section,Material,Options,dNodedx,dSectiondx)
...
A = norm(cross(Node(2,:)-Node(1,:),Node(3,:)-Node(1,:)))/2;
...
D = cmat_shell8(MatType,Material,k);  % RENAME FUNCTION TO CMAT_2DPLANESTRAIN ?
...
for iGauss=1:size(x,1)
    ...
    for zeta = [-xzeta(1),xzeta(1)]
        [Bg,J] = b_shell6(Ni,dN_dxi,dN_deta,zeta,Node,h,v1i,v2i,v3i,d);
        %B = theta*Bg;
        ...
        if nargout==2
            ...
            N(3,3:6:end) = Ni;
            if RotaryInertia == 1
                N(:,4:6:34) = (repmat(Ni,1,3).*(diag((zeta-zetan).*(h/2))*(-v2i))).';
                N(:,5:6:35) = (repmat(Ni,1,3).*(diag((zeta-zetan).*(h/2))*(v1i))).';
            end
...
Ke(6:6:end,6:6:end)=(1e-5)*E*Aelem*mean(h)*((8/7)*eye(6)-(1/7)*ones(6,6));
