function [Ax,Ay,Az,B,Cx,Cy,Cz,dAxdx,dAydx,dAzdx,dCxdx,dCydx,dCzdx]=disp_plane4(Nodes,Elements,DOF,EltIDDLoad,Sections,Materials,~,dNodesdx,dDLoadsdx,dSectionsdx)
...
Nx = sparse(4*nPoints+2,8);
Ny = sparse(4*nPoints+2,8);  
Nz = sparse(4*nPoints+2,8);
...
dofelem = kron(NodeNum(:),ones(2,1))+kron(ones(numel(NodeNum),1),[0.01;0.02]);
