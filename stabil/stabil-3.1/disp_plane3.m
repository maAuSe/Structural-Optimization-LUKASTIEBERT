function [Ax,Ay,Az,B,Cx,Cy,Cz,dAxdx,dAydx,dAzdx,dCxdx,dCydx,dCzdx]=disp_plane3(Nodes,Elements,DOF,EltIDDLoad,Sections,Materials,~,dNodesdx,dDLoadsdx,dSectionsdx)
...
for i=1:3*(nPoints)
  Ni=sh_t3(st(i,1),st(i,2));
  N(i,:)=Ni; 
end

Nx=sparse(3*nPoints+1,9);
Ny=sparse(3*nPoints+1,9);
Nz=sparse(3*nPoints+1,9);
