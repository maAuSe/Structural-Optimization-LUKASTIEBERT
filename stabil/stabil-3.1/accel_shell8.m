function DLoads=accel_shell8(Accelxyz,Nodes,Elements,Sections,Materials,Options)
...
for iElem=1:nElem
    ...
    if length(h)<4
        DLoads(iElem,:)=[Elements(iElem,1) -rho*kron(h(1,[1 1 1 1]),Accelxyz)];
    elseif any(isnan(h(2:4)))
        DLoads(iElem,:)=[Elements(iElem,1) -rho*kron(h(1,[1 1 1 1]),Accelxyz)];
    else
    DLoads(iElem,:)=[Elements(iElem,1) -rho*kron(h(1,1:4),Accelxyz)];
    end
end