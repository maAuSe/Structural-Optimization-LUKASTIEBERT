function [Ax,Ay,Az,B,Cx,Cy,Cz,dAxdx,dAydx,dAzdx,dCxdx,dCydx,dCzdx]=disp_plane6(Nodes,Elements,DOF,EltIDDLoad,Sections,Materials,nPoints,dNodesdx,dDLoadsdx,dSectionsdx)
...
for i = 1:3*(nPoints)
    Ni = sh_t6(st(i,1),st(i,2));
    N(i,:) = Ni; 
end

Nx = sparse(3*nPoints+1,18);
Ny = sparse(3*nPoints+1,18);
Nz = sparse(3*nPoints+1,18);
